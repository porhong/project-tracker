...
export function SprintMemberCapacityDialog({
  ...
  const planMatchesAvailability = members.every((member) => {
    const availableHours = memberAvailableHours(sprint, timeOffValues[member.id] ?? []);
    const allocatedHours = activeActivities.reduce(
      (total, activity) => total + (Number(allocationValues[`${member.id}:${activity.id}`]) || 0),
      0,
    );
    return Math.round(allocatedHours * 100) === Math.round(availableHours * 100);
  });
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[calc(100%-2rem)] overflow-y-auto sm:max-w-5xl"><DialogHeader><DialogTitle>Member capacity · {sprint.name}</DialogTitle><DialogDescription>Allocate each member’s total sprint hours and record work activity in one place. The allocation must exactly match available hours.</DialogDescription></DialogHeader><form action={formAction} className="space-y-5"><input type="hidden" name="id" value={sprint.id} /><input type="hidden" name="allocations" value={JSON.stringify(payload.allocations)} /><input type="hidden" name="time_off" value={JSON.stringify(payload.timeOff)} /><input type="hidden" name="activity_notes" value={JSON.stringify(payload.notes)} />{members.length === 0 ? <Alert><AlertDescription>Add active members to this project before creating its sprint capacity plan.</AlertDescription></Alert> : members.map((member) => {
