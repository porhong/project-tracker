...
export default async function SprintsPage({ searchParams }: SprintsPageProps) {
  ...
  const [{ data: profiles, error: profilesError }, { data: allocations, error: allocationsError }, { data: timeOff, error: timeOffError }, { data: activityNotes, error: activityNotesError }] = await Promise.all([
  ...
  const error = sprintsError ?? settingsError ?? activitiesError ?? projectMembersError ?? profilesError ?? allocationsError ?? timeOffError ?? activityNotesError;
